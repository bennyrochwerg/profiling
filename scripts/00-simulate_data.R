...
simulated_profiling_data <- tibble(
  # Simulating a total of 1,000 entries
  type = rep("Arrests", 1000),
  sex = sample(x = c("Male", "Female"),
               size = 1000,
               replace = TRUE),
  race = sample(x = c("Asian", "Black", "White", "Hawaiian/Pacific Islander",
...
simulated_profiling_data$sex |> class() == "character"
simulated_profiling_data$race |> class() == "character"
simulated_profiling_data$search |> class() == "character"
simulated_profiling_data$custody |> class() == "character"
...
sort(unique(simulated_profiling_data$sex |> na.omit())) == sort(c("Male",
                                                                  "Female"))
sort(unique(simulated_profiling_data$race |> na.omit())) ==
  sort(c("Asian", "Black", "White", "Hawaiian/Pacific Islander",
         "Middle Eastern", "Hispanic or Latino",
         "American Indian/Alaskan Native"))
sort(unique(simulated_profiling_data$search |> na.omit())) == sort(c("Yes",
                                                                     "No"))
